...
class Recent_Posts extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'Your site&#8217;s most recent Posts.', 'docly' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'recent-posts', esc_html__( '(Docly) Recent Posts', 'docly' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts', 'docly' );
		...
		foreach ( $r->posts as $recent_post ) :
            ...
            $title      = ( ! empty( $post_title ) ) ? $post_title : esc_html__( '(no title)', 'docly' );
            $title_length = function_exists('get_field') ? get_field( 'title_length',  'widget_'.$args['widget_id']) : 24;
            if ( !isset($title_length) ) {
                $title_length = 40;
            }
            ?>
            <div <?php post_class( 'media recent_post_item') ?>>
                <a href="<?php the_permalink( $recent_post->ID ); ?>">
                    <?php
                    if ( has_post_thumbnail($recent_post->ID) ) {
                        echo get_the_post_thumbnail( $recent_post->ID, 'docly_70x70', array( 'class' => 'media-object' ) );
                    }
                    ?>
                </a>
                <div class="media-body">
                    <a class="entry_post_date" href="<?php the_permalink( $recent_post->ID ); ?>" title="<?php echo esc_attr($title); ?>">
                        <h5> <?php docly_core_limit_latter($title, $title_length, ''); ?> </h5>
                    </a>
                    <?php
                    if ( $show_date ) : ?>
                        <a class="entry_post_date" href="<?php doclycore_day_link() ?>">
	...
	public function form( $instance ) {
        ...
        ?>
		<p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:', 'docly' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number of Posts to Show:', 'docly' ); ?></label>
        ...
        <p><input class="checkbox" type="checkbox"<?php esc_attr(checked( $show_date )); ?> id="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'show_date' )); ?>" />
            <label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display Post Date?', 'docly' ); ?></label></p>
        <?php
	}
